        ...
        data.forEach(plan => {
            ...
            procesados.forEach((bit, idx) => {
                ...
                this.planesData.push({
                    ...
                    comentarios: bit.NVO_COMENTARIOS || plan.COMENTARIOS
                });
            });
        });
        ...
        lineas.forEach(linea => {
            ...
            dias.forEach(dia => {
                ...
                if (eventos.length > 0) {
                    ...
                    const maxVisible = this.vistaActual === 'week' ? 100 : eventos.length;
        ...
        $(document).off('mouseenter.caltt', '.cal-pill')
            .on('mouseenter.caltt', '.cal-pill', (e) => {
                ...
                $('#calTooltipBody').html(`
                    ...
                    ${ev.comentarios ? `
                    <div class="cal-tt-row cal-tt-comentarios">
                        <span class="cal-tt-label">Comentarios</span>
                        <span>${String(ev.comentarios).replace(/\n/g, '<br>')}</span>
                    </div>
                    ` : ''}
                `);
